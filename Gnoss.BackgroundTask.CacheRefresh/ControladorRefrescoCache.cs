...
namespace Es.Riam.Gnoss.Win.RefrescoCache
{
    internal class ControladorRefrescoCache : ControladorServicioGnoss
    {
            ...
            using (var scope = ScopedFactory.CreateScope())
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(pFila))
                    {
                        ...
                        {
                            ProcesarFilaColaRefrescoCache(filaCola, entityContext, loggingService, redisCacheWrapper, servicesUtilVirtuosoAndReplication);
        ...
        private void ProcesarFilaColaRefrescoCache(BaseComunidadDS.ColaRefrescoCacheRow pFilaColaRefrescoCache, EntityContext entityContext, LoggingService loggingService, RedisCacheWrapper redisCacheWrapper, IServicesUtilVirtuosoAndReplication servicesUtilVirtuosoAndReplication)
        {
            EstaHiloActivo = true;
            ...
            if (pFilaColaRefrescoCache.TipoEvento == (short)TiposEventosRefrescoCache.ModificarCaducidadCache)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    EnviarCorreoErrorYGuardarLog(ex, "Error Refresco cach� (ProcesarFilaModificarCaducidadCache)", entityContext, loggingService);
                }
            }
            else
            {
                ...
                ProcesarFila(pFilaColaRefrescoCache, entityContext, loggingService, redisCacheWrapper, servicesUtilVirtuosoAndReplication);
            }
        }
            ...
            while (true)
            {
                using (var scope = ScopedFactory.CreateScope())
                {
                    ...
                    try
                    {
                        ...
                        //Esto lo tengo que hacer en un foreach aparte porque si no salta la excepci�n de Colleci�n modificada
        ...
        private void ProcesarFila(BaseComunidadDS.ColaRefrescoCacheRow pFilaCola, EntityContext entityContext, LoggingService loggingService, RedisCacheWrapper redisCacheWrapper, IServicesUtilVirtuosoAndReplication servicesUtilVirtuosoAndReplication)
        {
            switch (pFilaCola.TipoEvento)
            {
                case (short)TiposEventosRefrescoCache.BusquedaVirtuoso:
                    try
                    {
                        ProcesarFilaDeBusqueda(pFilaCola, entityContext, loggingService, redisCacheWrapper, servicesUtilVirtuosoAndReplication);
                    }
                    catch (ThreadAbortException) { }
                    catch (Exception ex)
                    {
                        pFilaCola.Estado = 1;
                        EnviarCorreoErrorYGuardarLog(ex, "Error Refresco cach� (ProcesarFilaDeBusqueda)", entityContext, loggingService);
                    }
        ...
        private void ProcesarFilaDeBusqueda(BaseComunidadDS.ColaRefrescoCacheRow pFilaCola, EntityContext entityContext, LoggingService loggingService, RedisCacheWrapper redisCacheWrapper, IServicesUtilVirtuosoAndReplication servicesUtilVirtuosoAndReplication)
        {
            ...
            if (filaParametroGeneral != null && filaParametroGeneral.PestanyaRecursosVisible)
            {
                ...
                {
                    // Por cada idioma recogeneramos la cach� de usuarios conectados, invitados y bots.
            ...
            {
                FacetadoCL facetadoCL = new FacetadoCL(mUrlIntragnoss, entityContext, loggingService, redisCacheWrapper, mConfigService, null, servicesUtilVirtuosoAndReplication, mLoggerFactory.CreateLogger<FacetadoCL>(), mLoggerFactory);
                ...
                //Si contiene el tipo de b�squeda que se est� pidiendo se necesitan limpiar todas las pestanyas que contengan esa b�squeda.
                ...
                foreach (ProyectoPestanyaBusqueda pestanyas in dataWrapperProyecto.ListaProyectoPestanyaBusqueda)
                {
                    ...
                    {
                        //Pasamos el par�metro UsuariosConPrivados = false para que limpie las claves de cach�.
        ...
        private void RegenerarCacheBusquedaUsuariosYBots(CargadorResultados pCargadorResultados, CargadorFacetas pCargadorFacetas, BaseComunidadDS.ColaRefrescoCacheRow pFilaCola, string pUbicacionBusqueda, string pIdioma, string pParametros_Adiccionales, LoggingService loggingService)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    pFilaCola.Estado = 1;
                    loggingService.GuardarLog("Error al refrescar los resultados de la fila " + pFilaCola.ColaID + " para el usuario conectado en idioma " + pIdioma + " ERROR:  Excepci�n: " + ex.ToString() + "\n\n\tTraza: " + ex.StackTrace, mlogger);
                }
                catch (Exception) { }
            }
            ...
            catch (Exception ex)
            {
                ...
                {
                    pFilaCola.Estado = 1;
                    loggingService.GuardarLog("Error al refrescar las facetas etiquetas de la fila " + pFilaCola.ColaID + " para el usuario conectado en idioma " + pIdioma + " ERROR:  Excepci�n: " + ex.ToString() + "\n\n\tTraza: " + ex.StackTrace, mlogger);
                }
                catch (Exception) { }
            }
            ...
            catch (Exception ex)
            {
                ...
                {
                    pFilaCola.Estado = 1;
                    loggingService.GuardarLog("Error al refrescar las facetas categorias de la fila " + pFilaCola.ColaID + " para el usuario conectado en idioma " + pIdioma + " ERROR:  Excepci�n: " + ex.ToString() + "\n\n\tTraza: " + ex.StackTrace, mlogger);
                }
                catch (Exception) { }
            }
            ...
            catch (Exception ex)
            {
                ...
                {
                    pFilaCola.Estado = 1;
                    loggingService.GuardarLog("Error al refrescar el resto de facetas de la fila " + pFilaCola.ColaID + " para el usuario conectado en idioma " + pIdioma + " ERROR:  Excepci�n: " + ex.ToString() + "\n\n\tTraza: " + ex.StackTrace, mlogger);
                }
                catch (Exception) { }
            }
            ...
            if (!pFilaCola.TipoBusqueda.Equals((short)TipoBusqueda.PersonasYOrganizaciones))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        pFilaCola.Estado = 1;
                        loggingService.GuardarLog("Error al refrescar las facetas de la home de la fila " + pFilaCola.ColaID + " para el usuario conectado en idioma " + pIdioma + " ERROR:  Excepci�n: " + ex.ToString() + "\n\n\tTraza: " + ex.StackTrace, mlogger);
                    }
                    catch (Exception) { }
                }
            }
            ...
            /*
                 * La cach� de resultados del usuario conectado y del usuari invitado se comparten (tienen la misma clave de cach�)
                 */
            //Usuario Invitado
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    pFilaCola.Estado = 1;
                    loggingService.GuardarLog("Error al refrescar los resultados de la fila " + pFilaCola.ColaID + " para el usuario Invitado en idioma " + pIdioma + " ERROR:  Excepci�n: " + ex.ToString() + "\n\n\tTraza: " + ex.StackTrace, mlogger);
                }
                catch (Exception) { }
            }
            ...
            catch (Exception ex)
            {
                ...
                {
                    pFilaCola.Estado = 1;
                    loggingService.GuardarLog("Error al refrescar las facetas etiquetas de la fila " + pFilaCola.ColaID + " para el usuario Invitado en idioma " + pIdioma + " ERROR:  Excepci�n: " + ex.ToString() + "\n\n\tTraza: " + ex.StackTrace, mlogger);
                }
                catch (Exception) { }
            }
            ...
            catch (Exception ex)
            {
                ...
                {
                    pFilaCola.Estado = 1;
                    loggingService.GuardarLog("Error al refrescar las facetas categorias de la fila " + pFilaCola.ColaID + " para el usuario Invitado en idioma " + pIdioma + " ERROR:  Excepci�n: " + ex.ToString() + "\n\n\tTraza: " + ex.StackTrace, mlogger);
                }
                catch (Exception) { }
            }
            ...
            catch (Exception ex)
            {
                ...
                {
                    pFilaCola.Estado = 1;
                    loggingService.GuardarLog("Error al refrescar el resto de facetas de la fila " + pFilaCola.ColaID + " para el usuario Invitado en idioma " + pIdioma + " ERROR:  Excepci�n: " + ex.ToString() + "\n\n\tTraza: " + ex.StackTrace, mlogger);
                }
                catch (Exception) { }
            }
            ...
            if (!pFilaCola.TipoBusqueda.Equals((short)TipoBusqueda.PersonasYOrganizaciones))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        pFilaCola.Estado = 1;
                        loggingService.GuardarLog("Error al refrescar las facetas de la home de la fila " + pFilaCola.ColaID + " para el usuario Invitado en idioma " + pIdioma + " ERROR:  Excepci�n: " + ex.ToString() + "\n\n\tTraza: " + ex.StackTrace, mlogger);
                    }
                    catch (Exception) { }
                }
            }
            ...
            catch (Exception ex)
            {
                ...
                {
                    pFilaCola.Estado = 1;
                    loggingService.GuardarLog("Error al refrescar los resultados de la fila " + pFilaCola.ColaID + " para el bot en idioma " + pIdioma + " ERROR:  Excepci�n: " + ex.ToString() + "\n\n\tTraza: " + ex.StackTrace, mlogger);
                }
                catch (Exception) { }
            }
            ...
            catch (Exception ex)
            {
                ...
                {
                    pFilaCola.Estado = 1;
                    loggingService.GuardarLog("Error al refrescar las facetas de la fila " + pFilaCola.ColaID + " para el bot en idioma " + pIdioma + " ERROR:  Excepci�n: " + ex.ToString() + "\n\n\tTraza: " + ex.StackTrace, mlogger);
                }
                catch (Exception) { }
            }
        ...
        #region Refresco de cach� de componentes
        ...
        private void RealizarPeticionWebRefrescoCache(CMSComponente pFilaComponente, EntityContext entityContext, LoggingService loggingService, RedisCacheWrapper redisCacheWrapper, IServicesUtilVirtuosoAndReplication servicesUtilVirtuosoAndReplication)
        {
            if (pFilaComponente != null)
            {
                try
                {
                    //Obtengo la url de refresco y hago una petici�n web
                    ...
                    {
                        //A�ado el dominio a la lista de peticiones
                        mPeticionesWebActuales.Add(urlHost, new List<string>());
                    }
                    else if (mPeticionesWebActuales[urlHost].Contains(urlRefresco))
                    {
                        //Esta petici�n ya est� en marcha, no la voy a hacer 2 veces porque el efecto ser�a el mismo
                        return;
                    }
                    ...
                    while (mPeticionesWebActuales[urlHost].Count >= mNumeroMaxPeticionesWebSimultaneas)
                    {
                        ...
                        {
                            //Hemos llegado al m�ximo de peticiones web concurrentes permitidas a este dominio, toca esperar hasta que termine alguna
                            Thread.Sleep(1000);
                            segundosEsperando++;
                        }
                        else
        ...
        //        {
        //            //Reviso la fecha de actualizaci�n, si ya se ha pasado, la fecha de actualizaci�n ser� ahora mismo
        //            DateTime fechaActualizacion = DateTime.Now;
        //            if (!filaComponente.IsFechaUltimaActualizacionNull() && filaComponente.FechaUltimaActualizacion > DateTime.Now)
        //            {
        //                //La fecha de actualizaci�n es posterior a este momento, lo pongo para que se actualice cuando le toque
